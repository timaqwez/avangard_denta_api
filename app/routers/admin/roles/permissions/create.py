...
class RolePermissionCreateByAdminSchema(BaseModel):
...
@router.post()
async def route(schema: RolePermissionCreateByAdminSchema):
    result = await RolePermissionService().create_by_admin(
        token=schema.token,
