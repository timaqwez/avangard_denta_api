...
from app.utils.exceptions.main import MethodNotSupportedRoot, NotEnoughPermissions
    ...
    def inner(function):
        async def wrapper(*args, **kwargs):
            ...
            if not session and not can_guest or (token and can_guest):
                ...
                if not session.account.is_active and session.id != 0:
                    raise NotEnoughPermissions(
                        kwargs={
                            'reason': 'Неактивный статус администратора.'
                        }
                    )
